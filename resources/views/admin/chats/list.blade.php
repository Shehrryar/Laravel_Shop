...
@section('customjs')
    <script>
        ...
        const allowedCustomerIds = @json($allowedCustomerIds ?? []);

        function canChatWithCustomer(userId) {
            if (!isVendor) {
                return true;
            }
            return allowedCustomerIds.map(Number).includes(Number(userId));
        }
        ...
        function selectUser(userId, userName) {
            if (!canChatWithCustomer(userId)) {
                alert('You cannot chat with another vendor customer.');
                return;
            }
        ...
        function subscribeAdminChat() {
            ...
            window.Echo.private(`chat.${adminId}`)
                .listen('.message.sent', function(data) {
                    ...
                    if (!canChatWithCustomer(customerId)) {
                        return;
                    }
