                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input value="{{ $size->name }}" type="text" name="name" id="name"
                                        ...
                                        @if ($products->isNotEmpty())
                                            @foreach ($products as $product)
                                                <option value="{{ $product->id }}"
                                                    {{ $size->product_id == $product->id ? 'selected' : '' }}>
                                                    {{ $product->title }}
                                                </option>
                                            @endforeach
                                        @endif
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response['status'] == true) {
                            ...
                            if (error['code']) {
                                $('#para_value').html(error['code']);
                            }

                            if (error['price']) {
                                $('#para_cod').html(error['price']);
                            }
                        }
                    },
