				...
				<div class="card-header">
					<div class="card-title">
						<button class="btn btn-default" type="button"
							onclick="window.location.href='{{route("sizes.index")}}'">Reset</button>
					</div>
							...
							@foreach($sizes as $size)
								<tr>
									...
									<a href="{{route('sizes.edit', $size->id)}}">
										...
										<a href="#" onclick='deleszie({{$size->id}})' class="text-danger w-4 h-4 mr-1">
...
@section('customjs') 
<script>
	function deleszie(size_id) {
		var delurl = '{{route("sizes.delete", "ID")}}'.replace("ID", size_id);
		if (confirm("Are you sure you want to delete " + size_id)) {
			$.ajax({
				url: delurl,
				type: 'delete',
